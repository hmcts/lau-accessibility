...
describe('@lau @challengedSpecific', () => {
  ...
  test('@checklist 5. Page Title is descriptive', async ({ page }) => {
    ...
    expect(['Challenged', 'Specific', 'Access'].some(word => title.toLowerCase().includes(word.toLowerCase()))).toBe(true);
  });
  ...
  test('@checklist @smoke 11. Error handling (LAU-1148)', async ({ page }) => {
    await cSAccess.clickSearchButton();
    const errorLink = await cSAccess.getErrorSummaryLink();
    await errorLink.click();
    const firstInput = await cSAccess.firstInputField();
